...
namespace Lab_8 {
    class MpiTestProgram {
        public static void DisplayRanks(string[] args) {
        ...
        public static void CircleMessaging(string[] args) {
            using (new MPI.Environment(ref args)) {
                Intracommunicator comm = Communicator.world;
                if (comm.Rank == 0) {
                    int[] dat = {1, 3, 2, 5};
        ...
        public static void Barrier(string[] args) {
            using (new MPI.Environment(ref args)) {
                Intracommunicator comm = Communicator.world;
                for (int i = 0; i < 5; i++) {
        ...
        public static void GatheringData(string[] args) {
            using (new MPI.Environment(ref args)) {
                Intracommunicator comm = Communicator.world;
        ...
        public static void MessageSpreading(string[] args) {
            using (new MPI.Environment(ref args)) {
                Intracommunicator comm = Communicator.world;
                string command = "null";
        ...
        public static void AllToAll(string[] args) {
            using (new MPI.Environment(ref args)) {
                Intracommunicator comm = Communicator.world;
